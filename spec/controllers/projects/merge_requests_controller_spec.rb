...
describe Projects::MergeRequestsController do
  ...
  describe 'POST #merge' do
    ...
    context 'when the sha parameter matches the source SHA' do
      def merge_with_sha
        post :merge, base_params.merge(sha: merge_request.source_branch_sha)
      end
      ...
      context 'when merge_when_build_succeeds is passed' do
        def merge_when_build_succeeds
          post :merge, base_params.merge(sha: merge_request.source_branch_sha, merge_when_build_succeeds: '1')
        end

        before do
          create(:ci_empty_pipeline, project: project, sha: merge_request.source_branch_sha, ref: merge_request.source_branch)
        end
